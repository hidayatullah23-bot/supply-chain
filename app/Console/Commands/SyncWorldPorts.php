...
class SyncWorldPorts extends Command
{
    ...
    {
        $url = config('services.world_ports.url');
        $offset = 0;
        $saved = 0;
        $rows = [];
        do {
            ...
            if (! $response->successful()) {
                $this->error('World Port Index tidak dapat diakses.');
            ...
            foreach ($features as $feature) {
                ...
                $g = $feature['geometry'] ?? [];
                $name = $a['PORT_NAME'] ?? $a['PORTNAME'] ?? $a['PORT_NAME_'] ?? $a['Main_Port_Name'] ?? $a['main_port_name'] ?? null;
                $country = $a['COUNTRY'] ?? $a['COUNTRY_NAME'] ?? $a['Country'] ?? $a['wpi_cc'] ?? 'Unknown';
                $lat = $g['y'] ?? $a['LATITUDE'] ?? null;
                $lng = $g['x'] ?? $a['LONGITUDE'] ?? null;
                if (! $name || $lat === null || $lng === null) {
                    continue;
                }$rows[] = ['port_name' => $name, 'country_name' => $country, 'latitude' => $lat, 'longitude' => $lng, 'harbor_size' => $a['HARBORSIZE'] ?? $a['HARBOR_SIZE'] ?? $a['harbor_size_code'] ?? null, 'source' => 'NGA World Port Index', 'created_at' => now(), 'updated_at' => now()];
            }
            $offset += count($features);
        } while (count($features) === 2000);
        if (! $rows) {
            $this->error('Tidak ada fitur WPI yang valid.');
        ...
        $this->info("{$saved} pelabuhan WPI disinkronkan.");
