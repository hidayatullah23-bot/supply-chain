            ...
            <section>
                ...
                <div class="row g-4" id="newsGrid">
                    @forelse($newsList as $news)
                        ...
                        @endphp
                        <div class="col-md-6 col-xxl-4 news-item" data-search="{{ strtolower($news->title.' '.$news->description) }}" data-sentiment="{{ $news->sentiment_status }}"><article class="news-card"><div class="news-image-wrap"><img src="{{ $image }}" class="news-image" alt="{{ $news->title }}" loading="lazy" onerror="this.onerror=null;this.src='{{ $fallback }}'"><span class="badge text-bg-{{ $color }} sentiment-badge">{{ $news->sentiment_status }}</span></div><div class="news-body"><div class="d-flex justify-content-between text-muted small mb-2"><span><i class="fa-regular fa-clock me-1"></i>{{ $news->created_at->diffForHumans() }}</span><span>+{{ $news->sentiment_score_positive }} / -{{ $news->sentiment_score_negative }}</span></div><div class="mb-2"><span class="badge text-bg-{{ $news->is_estimated?'secondary':'success' }}">{{ $news->is_estimated?'Baseline':'Berita aktual' }}</span> <small class="text-muted">{{ $news->data_source }}</small></div><h3 class="news-title">{{ $news->title }}</h3><p class="news-description">{{ \Illuminate\Support\Str::limit($news->description,180) }}</p><a href="{{ $news->source_url }}" target="_blank" rel="noopener" class="btn btn-dark btn-sm">Baca selengkapnya <i class="fa-solid fa-arrow-up-right-from-square ms-1"></i></a></div></article></div>
                    @empty
